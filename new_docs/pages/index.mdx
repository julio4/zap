...
Specifically, there's a lot of new potential [zkApp's use cases](./#use-cases) by being able to verify some kind of attestation based on private datas from *outside of Mina*.
...
- [Statement](./concepts/Statement): a logical expression that contains one or more variables and that can be either true or false.
- [Attestation](./concepts/Attestation): the truth value of a statement when all the variables of that statement are given a value.
