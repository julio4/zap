...
ZAP wants to abstract the complexity of handling private data during the generation of the attestation and the verification of the proof while ensuring that the logic rules are respected in a transparent way.
...
**Oracle** refers to what is used to a data source to get the value of the variables of the statement, it can be from on-chain data, off-chain data, ...
...
We are developing a demo Zap Oracle using [`Airstack`](https://www.airstack.xyz/) to bring Ethereum data on Mina, and more examples will be added in the near future.
