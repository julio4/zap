...
Each rule $r_i$ can involves any variables $v_i$ (atleast one), and can be expressed using logical operators ($\land$, $\lor$, $\lnot$) and comparison operators ($=$, $>$, $<$, $\geq$, $\leq$).
...
- If there's a variable $v_i$ that is not used in any rule, then $s$ is equivalent to the a statement that does not contain $v_i$ but has the same set of rules.
...
In ZAP, a statement is defined as an object with the following typescript interface:
