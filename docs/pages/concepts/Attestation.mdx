...
As explained in the [Statement](./Statement) part, the attestation is the proof that corresponds to verifying the truth value of a statement.
...
The `Attestation` struct contains the `statement` field as a [ProvableStatement](./Statement#provablestatement) and the `address` field as a `PublicKey` of the mina account that attests the statement.
